...
from pose_utils.datasets import create_dataloader
from pose_utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, scale_coords, xyxy2xywh, xywh2xyxy, set_logging, increment_path, colorstr
from pose_utils.metrics import ap_per_class, ConfusionMatrix
from pose_utils.plots import plot_images, output_to_target, plot_study_txt
from pose_utils.torch_utils import select_device, time_synchronized
import cv2
...
def test(data,
    ...
    if training:  # called by train.py
        ...
        (save_dir / 'labels' if save_txt or save_txt_tidl else save_dir).mkdir(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        if dump_img:
            dst_file = os.path.join(
                save_dir, 'dump_img', 'images', 'val2017', Path(paths[0]).stem + '.png')
            os.makedirs(os.path.dirname(dst_file), exist_ok=True)
            cv2.imwrite(dst_file, img[0].numpy(
            ).transpose(1, 2, 0)[:, :, ::-1])
        # print(img.shape)
        ...
        with torch.no_grad():
            ...
            if flip_test:
                ...
                # inference and training outputs
                out_flip, train_out_flip = model(img_flip, augment=augment)
                model.model[-1].flip_test = False
            ...
            if kpt_label:
                ...
                # to pixels
                targets[:,
                        2:] *= torch.Tensor([width, height]*num_points).to(device)
            else:
                # to pixels
            ...
            out = non_max_suppression(out, conf_thres, iou_thres, labels=lb, multi_label=True,
                                      agnostic=single_cls, kpt_label=kpt_label, nc=model.yaml['nc'], nkpt=model.yaml['nkpt'])
        ...
        for si, pred in enumerate(out):
            ...
            scale_coords(img[si].shape[1:], predn[:, :4], shapes[si]
                         [0], shapes[si][1], kpt_label=False)  # native-space pred
            if kpt_label:
                scale_coords(img[si].shape[1:], predn[:, 6:], shapes[si][0],
                             shapes[si][1], kpt_label=kpt_label, step=3)  # native-space pred
            # Append to text file
            if save_txt:
            ...
            if save_txt_tidl:  # Write to file in tidl dump format
                for *xyxy, conf, cls in predn[:, :6].tolist():
                    ...
                    # label format
                    line = (conf, cls, *xyxy) if opt.save_conf else (cls, *xyxy)
                    with open(save_dir / 'labels' / (path.stem + '.txt'), 'a') as f:
                        f.write(('%g ' * len(line)).rstrip() % line + '\n')
            ...
            if len(wandb_images) < log_imgs and wandb_logger.current_epoch > 0:  # Check for test operation
                if wandb_logger.current_epoch % wandb_logger.bbox_interval == 0:
                    ...
                    wandb_images.append(wandb_logger.wandb.Image(
                        img[si], boxes=boxes, caption=path.name))
            wandb_logger.log_training_progress(
                predn, path, names) if wandb_logger and wandb_logger.wandb_run else None
            ...
            if nl:
                ...
                # native-space labels, kpt_label is set to False as we are dealing with boxes here
                scale_coords(img[si].shape[1:], tbox, shapes[si]
                             [0], shapes[si][1], kpt_label=False)
                if kpt_label:
                    tkpt = labels[:, 5:]
                    # native-space labels
                    scale_coords(img[si].shape[1:], tkpt, shapes[si]
                                 [0], shapes[si][1], kpt_label=kpt_label)
    ...
    if plots:
        ...
        if wandb_logger and wandb_logger.wandb:
            val_batches = [wandb_logger.wandb.Image(
                str(f), caption=f.name) for f in sorted(save_dir.glob('test*.jpg'))]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-txt-tidl', action='store_true',
                        help='save results to *.txt in tidl format')
    parser.add_argument('--tidl-load', action='store_true',
                        help='load thedata from a list specified as in tidl')
    parser.add_argument('--dump-img', action='store_true',
                        help='load thedata from a list specified as in tidl')
    ...
    parser.add_argument('--save-json-kpt', action='store_true',
                        help='save a cocoapi-compatible JSON results file for key-points')
    parser.add_argument('--project', default='runs/test',
                        help='save to project/name')
    parser.add_argument('--name', default='exp', help='save to project/name')
    parser.add_argument('--exist-ok', action='store_true',
                        help='existing project/name ok, do not increment')
    parser.add_argument('--kpt-label', action='store_true',
                        help='Whether kpt-label is enabled or not')
    parser.add_argument('--flip-test', action='store_true',
                        help='Whether to run flip_test or not')
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        for w in opt.weights:
            test(opt.data, w, opt.batch_size, opt.img_size, 0.25,
                 0.45, save_json=False, plots=False, opt=opt)
