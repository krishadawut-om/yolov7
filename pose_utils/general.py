...
from pose_utils.google_utils import gsutil_getsize
from pose_utils.metrics import fitness
from pose_utils.torch_utils import init_torch_seeds
...
def download(url, dir='.', threads=1):
    ...
    if threads > 1:
        ThreadPool(threads).imap(lambda x: download_one(
            *x), zip(url, repeat(dir)))  # multi-threaded
    else:
        ...
        for kpt in range(num_kpts):
            for kpt_instance in range(y.shape[0]):
                if y[kpt_instance, 2 * kpt + 4] != 0:
                    y[kpt_instance, 2*kpt+4] = w * \
                        y[kpt_instance, 2*kpt+4] + padw
                if y[kpt_instance, 2 * kpt + 1 + 4] != 0:
                    y[kpt_instance, 2*kpt+1+4] = h * \
                        y[kpt_instance, 2*kpt+1+4] + padh
    return y
...
def non_max_suppression_export(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    min_wh, max_wh = 2, 4096
    xc = prediction[..., 4] > conf_thres  # candidates
    output = [torch.zeros((0, 57), device=prediction.device)
              ] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        x = x[xc[xi]]  # confidence
        # Compute conf
