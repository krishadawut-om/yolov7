from pose_utils.general import colorstr, xywh2xyxy, check_dataset
from pose_utils.datasets import img2label_paths
from pose_utils.datasets import LoadImagesAndLabels
import json
import sys
...
def check_wandb_config_file(data_config_file):
    wandb_config = '_wandb.'.join(
        data_config_file.rsplit('.', 1))  # updated data.yaml path
...
def check_wandb_resume(opt):
    process_wandb_config_ddp_mode(
        opt) if opt.global_rank not in [-1, 0] else None
        ...
        if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
            if opt.global_rank not in [-1, 0]:  # For resuming DDP runs
                ...
                artifact = api.artifact(
                    project + '/' + model_artifact_name + ':latest')
...
def process_wandb_config_ddp_mode(opt):
    ...
    if isinstance(data_dict['train'], str) and data_dict['train'].startswith(WANDB_ARTIFACT_PREFIX):
        api = wandb.Api()
        train_artifact = api.artifact(remove_prefix(
            data_dict['train']) + ':' + opt.artifact_alias)
    ...
    if isinstance(data_dict['val'], str) and data_dict['val'].startswith(WANDB_ARTIFACT_PREFIX):
        api = wandb.Api()
        val_artifact = api.artifact(remove_prefix(
...
class WandbLogger():
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
                ...
                self.wandb_run = wandb.init(
                    id=run_id, project=project, resume='allow')
    ...
    def download_model_artifact(self, opt):
        if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
            model_artifact = wandb.use_artifact(remove_prefix(
                opt.resume, WANDB_ARTIFACT_PREFIX) + ":latest")
