...
from pose_utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from pose_utils.torch_utils import torch_distributed_zero_first
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            self.img_files = [x.replace('/', os.sep).split(' ')[0]
                              for x in f if x.split(' ')[0].split('.')[-1].lower() in img_formats]
            sorted_index = [i[0] for i in sorted(
                enumerate(self.img_files), key=lambda x:x[1])]
            self.img_files = [self.img_files[index] for index in sorted_index]
            if self.tidl_load:
                self.img_sizes = [x.replace('/', os.sep).split(' ')[2].split(',')
                                  for x in f if x.split(' ')[0].split('.')[-1].lower() in img_formats]
                self.img_sizes = [self.img_sizes[index]
                                  for index in sorted_index]
                self.img_sizes = [[int(dim_size) for dim_size in img_size]
        ...
        if cache_path.is_file():
            ...
            if cache['hash'] != get_hash(self.label_files + self.img_files) or 'version' not in cache:
                cache, exists = self.cache_labels(
                    cache_path, prefix, self.kpt_label), False  # re-cache
        else:
            cache, exists = self.cache_labels(
                cache_path, prefix, self.kpt_label), False  # cache
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        ne += 1  # label empty
                        l = np.zeros((0, 39), dtype=np.float32) if kpt_label else np.zeros(
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                labels[:, 1:] = xywhn2xyxy(
                    labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1], kpt_label=self.kpt_label)
        ...
        labels_out = torch.zeros(
            (nL, 6+2*num_kpts)) if self.kpt_label else torch.zeros((nL, 6))
        if nL:
            if self.kpt_label:
        ...
        if use_segments:  # warp segments
            ...
            if kpt_label:
                ...
                xy_kpts[:, :2] = targets[:, 5:].reshape(
                    n*17, 2)  # num_kpt is hardcoded to 17
                xy_kpts = xy_kpts @ M.T  # transform
                # perspective rescale or affine
                xy_kpts = (xy_kpts[:, :2] / xy_kpts[:, 2:3]
                           if perspective else xy_kpts[:, :2]).reshape(n, 34)
                xy_kpts[targets[:, 5:] == 0] = 0
                x_kpts = xy_kpts[:, list(range(0, 34, 2))]
                y_kpts = xy_kpts[:, list(range(1, 34, 2))]

                x_kpts[np.logical_or.reduce(
                    (x_kpts < 0, x_kpts > width, y_kpts < 0, y_kpts > height))] = 0
                y_kpts[np.logical_or.reduce(
                    (x_kpts < 0, x_kpts > width, y_kpts < 0, y_kpts > height))] = 0
