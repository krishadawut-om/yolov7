...
from pose_utils.general import bbox_iou
from pose_utils.torch_utils import is_parallel
...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        lcls, lbox, lobj, lkpt, lkptv = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(
            1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        sigmas = torch.tensor([.26, .25, .25, .35, .35, .79, .79, .72, .72, .62, .62,
                              1.07, 1.07, .87, .87, .89, .89], device=device) / 10.0
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.kpt_label:
                    ...
                    kpt_loss_factor = (
                        torch.sum(kpt_mask != 0) + torch.sum(kpt_mask == 0))/torch.sum(kpt_mask != 0)
        ...
        for i in range(self.nl):
            ...
            if self.kpt_label:
                for kpt in range(self.nkpt):
                    t[:, 6+2*kpt: 6+2*(kpt+1)][t[:, 6+2*kpt: 6+2*(kpt+1)]
                                               != 0] -= gij[t[:, 6+2*kpt: 6+2*(kpt+1)] != 0]
                tkpt.append(t[:, 6:-1])
