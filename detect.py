...
from pose_utils.datasets import LoadStreams, LoadImages
from pose_utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
    scale_coords, xyxy2xywh, strip_optimizer, set_logging, increment_path, save_one_box
from pose_utils.plots import colors, plot_one_box
from pose_utils.torch_utils import select_device, load_classifier, time_synchronized
...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres,
                                   classes=opt.classes, agnostic=opt.agnostic_nms, kpt_label=kpt_label)
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        plot_one_box(xyxy, im0, label=label, color=colors(
                            c, True), line_thickness=opt.line_thickness, kpt_label=kpt_label, kpts=kpts, steps=3, orig_shape=im0.shape[:2])
                        if opt.save_crop:
                            save_one_box(
                ...
                if save_txt_tidl:  # Write to file in tidl dump format
                    for *xyxy, conf, cls in det_tidl:
                        ...
                        # label format
                        line = (conf, cls,  *
                                xyxy) if opt.save_conf else (cls, *xyxy)
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(('%g ' * len(line)).rstrip() % line + '\n')
